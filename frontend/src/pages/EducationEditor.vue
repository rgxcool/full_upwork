<template>
  <div class="hierarchy-manager">
    ...
    <!-- Program Selection -->
    <div>
      <v-select v-model="selectedProgram" :items="programs" item-title="programName" item-value="_id"
        placeholder="Select program" @update:modelValue="fetchAllCourses" class="styled-select" />
    </div>
    <br />
    <br />
    <!-- Add Individual Course to Student -->
    <div>
      <v-select v-model="selectedIndividualCourse" :items="allCourses" item-title="displayText" item-value="_id"
        placeholder="Select course" class="styled-select" />
      <br />
      <!-- <input id="student-name" v-model="studentName" placeholder="Student Name" class="styled-input"/> -->
      <v-container>
        <v-form>
          <!-- Dropdown for selecting a student -->
          <v-select v-model="selectedStudent" :items="students" item-title="namn" item-value="_id"
            label="Select a student" return-object outlined />
        </v-form>
      </v-container>
      <br />
      <br />
      <button class="btn btn-primary" @click="handleAddCourse">
...
export default {
  ...
  methods: {
    ...
    async fetchPrograms() {
      try {
        const response = await axios.get(`${process.env.VUE_APP_API_URL}/api/programs`);
      ...
      if (this.selectedProgram) {
        try {
          ...
          const response = await axios.get(
            `${process.env.VUE_APP_API_URL}/api/programs/${this.selectedProgram}/courses`
          );
    ...
    async addCourseToStudent() {
      try {
        ...
        await axios.post(
          `${process.env.VUE_APP_API_URL}/api/students/${this.studentId}/courses`,
    ...
    async addProgramToStudent() {
      try {
        ...
        const response = await axios.post(
          `${process.env.VUE_APP_API_URL}/api/students/${this.studentId}/programs`,
