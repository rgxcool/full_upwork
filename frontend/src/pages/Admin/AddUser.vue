...
export default {
  ...
  methods: {
    ...
    async submitStudentForm() {
      try {
        const response = await axios.post(`${process.env.VUE_APP_API_URL}/api/students`, this.studentForm);
    ...
    async submitTeacherForm() {
      try {
        const response = await axios.post(`${process.env.VUE_APP_API_URL}/api/teacher`, this.teacherForm);
