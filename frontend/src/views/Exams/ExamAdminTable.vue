<template>
      ...
      <table class="table table-bordered table-striped align-middle">
        ...
        <tbody>
          <tr v-for="exam in filteredExams" :key="exam._id">
            ...
            <td>{{ exam.status }}</td>
            <td>
              <span v-if="exam.materialReceived" class="badge bg-success">Ja</span>
  ...
  const filteredExams = computed(() => {
    return exams.value.filter(e => {
      ...
      const matchMonth = !filterMonth.value || e.requestedMonth === filterMonth.value
  ...
  const submitDecision = async (id) => {
    ...
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}/api/exams/${id}/decision`, {
        decision,
        comment
      })
      alert('Beslut sparat')
