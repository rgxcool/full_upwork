<template>
  <div class="container">
    <div v-if="uploadSuccess" class="alert alert-success alert-dismissible fade show" role="alert"
        ...
        <tbody>
          <tr v-for="student in filteredStudents" :key="student._id" :class="{ 'dropout-row': student.dropout }">
              ...
              <div class="course-list">
                <ul v-if="student.education && student.education.length">
                  <li v-for="(edu, index) in student.education" :key="'edu-' + index">
                    ...
                    <span v-if="edu.grade"><strong>({{ edu.grade }})</strong></span>
                  </li>
                </ul>
            ...
            <dialog v-if="editingStudent" class="edit-dialog" open>
              ...
              <form @submit.prevent="saveEditedStudent">
                ...
                <div v-for="(edu, index) in editingStudent.education" :key="index" class="mb-2">
                  <v-autocomplete v-model="editingStudent.education[index]" :items="educationOptions" item-title="name"
                    return-object label="Add or edit education" class="mb-2" />

                  <!-- Grade dropdown for courses -->
                  <v-select v-if="edu.type === 'Course'" v-model="edu.grade" :items="['A', 'B', 'C', 'D', 'E', 'F']"
                    label="Select grade" class="mt-2" />
                ...
                <textarea v-model="editingStudent.additionalInfo" placeholder="Info" rows="3"></textarea>
            ...
            <td class="dropout-column">
              <button :class="['dropout-btn', student.dropout ? 'dropout-active' : 'dropout-inactive']"
            ...
            <td>
              ...
              <button class="btn btn-danger btn-xs delete-single-button" @click="deleteStudent(student._id)">
                Delete
              </button>
            </td>
          </tr>
    ...
    async fetchEducationOptions() {
      try {
        const [programsRes, packagesRes, coursesRes] = await Promise.all([
          axios.get(`${import.meta.env.VITE_API_URL}/api/all-programs`, { withCredentials: true }),
