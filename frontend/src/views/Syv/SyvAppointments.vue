  ...
  methods: {
    async fetchMeetings() {
      try {
        ...
        const response = await axios.get(`/api/meetings?bookedBy=syv&sort=start:desc&page=${page}&limit=${limit}`);
        this.meetings = response.data.data;
        this.pagination = response.data.pagination;
        this.isModalOpen = false; // Close modal on success
      } catch (error) {
        console.error("Kunde inte hämta SYV-samtal:", error);
      }
    },
